...
function M.cache.file(key)
  return vim.fs.joinpath(vim.fn.stdpath("cache"), "rwth-" .. key .. ".json")
end

--- Safely read and decode the cached file from disk
---@param key string
---@return rwth.Cache|nil
...
---@param key string
---@param data rwth.Cache
function M.cache.write(key, data)
  pcall(M.write, M.cache.file(key), vim.json.encode(data))
end

--- Deletes rwth's cache files from the system
function M.cache.clear()
  local files = vim.fn.glob(vim.fn.stdpath("cache") .. "/rwth-*.json", false, true)
...
---@param groups rwth.Highlights
---@return rwth.Highlights
...
function M.reload()
  ...
  for name, _ in pairs(package.loaded) do
    if name:match("^rwth") and name ~= "rwth.config" then
...
function M.resolve(theme)
  ...
  local ret = require("rwth.config").options.theme[vim.o.background] or vim.o.background
  return ret
end
